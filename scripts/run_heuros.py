...
mst_configs = [
    ...
    # dihedral + area
    MSTConfig(-1, -1, 0),  # max both
    MSTConfig(1, 1, 0),  # min both
    MSTConfig(-1, 1, 0),  # max dihedral, min area
    MSTConfig(1, -1, 0),  # min dihedral, max area
    # dihedral + centroid
    MSTConfig(-1, 0, -1),  # max both
    MSTConfig(1, 0, 1),  # min both
    MSTConfig(-1, 0, 1),  # max dihedral, min centroid
    MSTConfig(1, 0, -1),  # min dihedral, max centroid
    # area + centroid
    MSTConfig(0, -1, -1),  # max both
    MSTConfig(0, 1, 1),  # min both
    MSTConfig(0, -1, 1),  # max area, min centroid
    MSTConfig(0, 1, -1),  # min area, max centroid
    # all three
    MSTConfig(-1, -1, -1),  # max all
    MSTConfig(1, 1, 1),  # min all
    MSTConfig(-1, -1, 1),  # max dihedral, max area, min centroid
    MSTConfig(-1, 1, -1),  # max dihedral, min area, max centroid
    MSTConfig(1, -1, -1),  # min dihedral, max area, max centroid
    MSTConfig(1, 1, -1),  # min dihedral, min area, max centroid
    MSTConfig(1, -1, 1),  # min dihedral, max area, min centroid
    MSTConfig(-1, 1, 1),  # max dihedral, min area, min centroid
]

traversal_configs = [Dfs, Bfs, RandomSpanningTree]
...
def process_batch(args):
    ...
    for i in range(batch_start, batch_start + 100):
        ...
        try:
            polytope = pb.deserialize(
                DATASET_DIR / distribution / facet_dir / f"polytope_{i}.json"
            )
        ...
        for config in priority_configs:
            try:
                ...
                polytope_results[str(config)] = bool(
        ...
        for cls in traversal_configs:
            try:
                ...
                polytope_results[cls.id()] = bool(polytope.overlap_free_unfolding(st))
            except Exception as e:
                print(e)
                polytope_results[cls.id()] = None
